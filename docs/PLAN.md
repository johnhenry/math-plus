...
| `tensor-webgpu` | `tensor-core` **+** `tensor-compile` | Large matmuls/attention-adjacent/image kernels need the fusion/IR layer for GPU dispatch planning | ✅ Shipped (#12) — GEMM/attention/elementwise-fusion WGSL kernels verified against a real headless GPUAdapter; GEMM now tiled / skinny / subgroup-matrix with f16 storage + f32 accumulation, crossover re-measured at 128x128 on an Apple M2 (`docs/spikes/webgpu-tiled-gemm.md`; v1's naive kernel never crossed, `docs/spikes/webgpu-baseline.md`); Node/Bun via Dawn (`./dawn` subpath) |
